...
function timeSpanToString(startDate, endDate) {
  ...
  const h = Math.trunc((result / 3600000) % 100)
    .toString()
    .padStart(2, '0');
  const m = Math.trunc((result / 60000) % 60)
    .toString()
    .padStart(2, '0');
  const s = Math.trunc((result / 1000) % 60)
    .toString()
    .padStart(2, '0');
  const ms = Math.trunc(result % 1000)
    .toString()
    .padStart(3, '0');
  return `${h}:${m}:${s}.${ms}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const ang = Math.abs(hoursAngle - minutesAngle);

  return (Math.PI * Math.min(360 - ang, ang)) / 180;
}
